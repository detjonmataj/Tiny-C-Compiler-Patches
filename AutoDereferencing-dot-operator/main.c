...
int main(void)
{
    ...
    ///! It will access struct fields with member access operator '->' in the regular way (not Auto Deferecing with dot operator)
    sum(&foo);

    ///! Calling the function _sum with the address of foo as argument
    ///! This function will add the values of f1 and f2 and print the sum
    ///! It will access struct fields with member access operator '.' using Auto Deferecing with dot operator Patch
